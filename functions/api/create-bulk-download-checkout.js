/**
 * Cloudflare Pages Function: Create Stripe Checkout for Bulk Song Downloads
 * Endpoint: /api/create-bulk-download-checkout
 */
    ...
    try {
        ...
        const session = await fetch('https://api.stripe.com/v1/checkout/sessions', {
            ...
            body: new URLSearchParams({
                ...
                'line_items[0][price_data][product_data][name]': 'Download: All Songs',
                'line_items[0][price_data][product_data][description]': 'Complete collection of high-quality MP3 downloads + support for Fathers Son Music ministry',
                'line_items[0][price_data][unit_amount]': (amount * 100).toString(),
                'line_items[0][quantity]': '1',
                'success_url': `${new URL(context.request.url).origin}/success.html?session_id={CHECKOUT_SESSION_ID}&bulk=true`,
                'cancel_url': `${new URL(context.request.url).origin}/#songs`,
                'metadata[firstName]': firstName,
                'metadata[lastName]': lastName,
                'metadata[bulkDownload]': 'true'
            })
        });
